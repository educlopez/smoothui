...
image: "/blog/framer-motion-tutorial.webp"
---
...
Every component in [SmoothUI](https://smoothui.dev) follows this pattern. The library enforces `useReducedMotion` checks across all {COMPONENT_COUNT} components, so you don't have to remember it yourself.
...
If you're building a React app with Tailwind CSS and want polished animations without writing them from scratch, [SmoothUI](https://smoothui.dev) provides {COMPONENT_COUNT} production-ready components built on Motion. Every component uses springs, respects reduced motion, and installs via the shadcn registry:
