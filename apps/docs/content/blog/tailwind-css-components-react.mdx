...
**{COMPONENT_COUNT} animated React components** built with Motion (Framer Motion), Tailwind CSS v4, and React 19. Fully shadcn-compatible — uses the same registry system, same `cn()` utility, same copy-paste model.
...
- **{BLOCK_COUNT} pre-built blocks** — hero sections, pricing tables, testimonials, footers
...
Browse the [SmoothUI component library](https://smoothui.dev/docs/components) to see all {COMPONENT_COUNT} animated components, or start with the [installation guide](https://smoothui.dev/docs/guides/getting-started).
