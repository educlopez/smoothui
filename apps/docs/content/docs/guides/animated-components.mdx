...
description: Explore 50+ animated React components built with Motion (Framer Motion). Smooth animations, hover effects, transitions, and interactive UI elements for modern web apps.
icon: Sparkles
---
...
<Accordions>
<Accordion id="motion-required" title="Do I need to know Motion/Framer Motion?">
  No! All animations are built-in and work out of the box. You can use components without writing any animation code. For customization, basic Motion knowledge helps but isn't required.
</Accordion>
<Accordion id="bundle-size" title="How do animations affect bundle size?">
  Motion is the only animation dependency. Components are tree-shakeable, so you only ship the code you use. Typical impact is 20-40KB for Motion plus component code.
</Accordion>
